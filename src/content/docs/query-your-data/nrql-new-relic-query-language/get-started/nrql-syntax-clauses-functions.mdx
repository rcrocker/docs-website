...
<CollapserGroup>
  ...
  <Collapser
    className="freq-link"
    id="facet-as"
    /<><InlineCode>FACET ... AS</InlineCode> clause</>}
  >
  ...
  >
    ...
    * The cardinality of the join is limited to 1:100, meaning a single join key cannot map to more
    ...
    <CollapserGroup>
      <Collapser title={<><InlineCode>INNER JOIN</InlineCode> with a faceted subquery</>}
...
Note: `aparse()` is case-insensitive.
  ...
  <Collapser
    className="freq-link"
    id="func-histogram"
    title={<InlineCode>histogram(attribute, [ceiling [,number of buckets [, start]]])</InlineCode>}
  >
    Use the `histogram()` function to generate histograms. It takes up to four arguments:

    * Attribute name (required)
    * Maximum value of the sample range (`width`, default value: 10.0). Attribute values that exceed the maximum value will be included in the last bucket.
    * Total number of buckets (`buckets`, between 1 and 500, inclusive, default value: 40).
    * Starting value for the histogram (`start`, default value: 0). Attribute values below the starting value are not included in the histogram.
      ...
      <CollapserGroup>
        ...
        <Collapser
          id="histogram-start"
          title="Using a start value to better see higher quantiles"
        >
          `histogram()` accepts a starting value to provide insight into higher quantiles. This query uses a starting value of 5 to
          show the 20 buckets for response times in the 5 to 10 second range. As above. response time values greater than `width` are counted in the final bucket.

          ```sql
          SELECT histogram(myDistributionMetric, 10, 20, 5) FROM Metric SINCE 1 week ago
          ```
        </Collapser>
